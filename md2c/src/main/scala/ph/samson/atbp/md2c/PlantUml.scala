  ...
  def render(adf: Doc): ZIO[Any, Throwable, Map[String, File]] = {
    def renders(outDir: File) = adf
      ...
      .map { (codeBlock, index) =>
        ...
        val plantuml =
          if source.trim.startsWith("@start") then source
          else s"""@startuml
                  |$source
                  |@enduml""".stripMargin
        ...
        val fileFormat = codeBlock
          .language()
          .map(l =>
            if l.contains(".svg") then FileFormat.SVG
            else if l.contains(".png") then FileFormat.PNG
            else FileFormat.SVG
          )
  ...
  def transform(adf: Doc): Task[Doc] = {
    def transformer(renders: Map[String, File]): DocContent => DocContent = {
      case codeBlock: CodeBlock =>
        if codeBlock.language().orElse("").startsWith("plantuml") then {
          val source = codeBlock.toPlainText
